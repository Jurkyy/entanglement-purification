...
def three_to_one_protocol_alice(q1, q2, q3, alice, socket):
    ...
    socket.send(str(outcome1) + str(outcome2))
    b = socket.recv()

    return b == (str(outcome1) + str(outcome2))
...
def three_to_one_protocol_bob(q1, q2, q3, bob, socket):
    ...
    return a == (str(outcome1) + str(outcome2))
...
def bell_measurement(q1, q2):
    """
    Performs a Bell measurement on qubit 1 and 2
    :param q1: First qubit to measure
    :param q2: Second qubit to measure
    :return: The pair of outputs
    """
